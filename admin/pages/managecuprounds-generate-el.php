			...
			<table class="table table-striped table-hover">
				...
				<tbody>
					...
					$result = $db->querySelect("T.id AS team_id,T.name AS team_name,L.name AS league_name,L.land AS league_country",
                            ...
                            cm23_el_temp AS CL",
							"T.id=CL.verein_id ORDER BY team_name ASC");
				    
					/*$result = $db->querySelect("verein_id",
					$website->getConfig("db_prefix") . "_cl_temp",
					"1=1 ORDER BY RAND()");*/
		
					while ($team = $result->fetch_array()) {
						echo "<tr>";
						echo "<td><input type=\"checkbox\" class=\"teamForCupCheckbox\" name=\"teams[]\" value=\"". $team["team_id"] . "\" checked=checked></td>";
