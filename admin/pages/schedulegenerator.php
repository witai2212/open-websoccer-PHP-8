...
elseif ($show == "generate") {
  ...
  //##### Save #####
		...
		$leagues = LeagueDataService::getLeaguesSortedByCountry($website, $db);
		foreach ($leagues as $league) {
			
			$league_id = $league['league_id'];

			// get teams
			$result = $db->querySelect("id", $website->getConfig("db_prefix") . "_verein", "liga_id = %d", $league_id);
			if (!$result->num_rows) {
			...
			$seasoncolumns["liga_id"] = $league_id;
