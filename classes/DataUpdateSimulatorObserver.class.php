...
class DataUpdateSimulatorObserver implements ISimulatorObserver {
	...
	public function onMatchCompleted(SimulationMatch $match) {
		...
		if ($isFriendlyMatch) {
			...
			if (!$match->homeTeam->isNationalTeam) {
				$this->creditSponsorPayments($match, $match->homeTeam, TRUE, $match->homeTeam->getGoals() > $match->guestTeam->getGoals());
			}
			if (!$match->guestTeam->isNationalTeam) {
				$this->creditSponsorPayments($match, $match->guestTeam, FALSE, $match->homeTeam->getGoals() < $match->guestTeam->getGoals());
			}
	...
	private function updatePlayers(SimulationMatch $match, SimulationTeam $team, $isTeamWinner, $isTie) {
		...
		$pcolumns = 'id,vorname,nachname,kunstname,verein_id,position,age,vertrag_spiele,vertrag_gehalt,vertrag_torpraemie,w_staerke,w_technik,w_kondition,w_frische,w_zufriedenheit,w_talent,w_passing,w_shooting,w_heading,w_tackling,w_freekick,w_pace,w_creativity,w_influence,w_flair,w_penalty,w_penalty_killing,verletzt,gesperrt,gesperrt_cups,gesperrt_nationalteam,lending_fee,lending_matches,lending_owner_id,personality';
		$fromTable = $this->_websoccer->getConfig('db_prefix') . '_spieler';
	...
	private function updatePlayer(SimulationMatch $match, SimulationPlayer $player, $isTeamWinner, $isTie) {
		...
		$pcolumns = 'id,vorname,nachname,kunstname,verein_id,position,age,vertrag_spiele,vertrag_gehalt,w_staerke,w_technik,w_kondition,w_frische,w_zufriedenheit,w_talent,w_passing,w_shooting,w_heading,w_tackling,w_freekick,w_pace,w_creativity,w_influence,w_flair,w_penalty,w_penalty_killing,st_tore,st_assists,st_spiele,st_karten_gelb,st_karten_gelb_rot,st_karten_rot,sa_tore,sa_assists,sa_spiele,sa_karten_gelb,sa_karten_gelb_rot,sa_karten_rot,lending_fee,lending_owner_id,lending_matches,personality';
		$result = $this->_db->querySelect($pcolumns, $fromTable, $whereCondition, $parameters);
		...
		if (!$player->team->isNationalTeam || $this->_websoccer->getConfig('sim_playerupdate_through_nationalteam')) {
			...
			if ($player->getMinutesPlayed() >= $minMinutes) {
				$columns['w_kondition'] = min(100, $player->strengthStamina + $staminaChange);
				$satisfactionDelta = PlayerPersonalityDataService::adjustSatisfactionDelta($player->personality, $satisfactionChange, 'played');
				$columns['w_zufriedenheit'] = min(100, max(1, $player->strengthSatisfaction + $satisfactionDelta));
			} else {
				$columns['w_kondition'] = max(1, $player->strengthStamina - $staminaChange);
				$satisfactionDelta = PlayerPersonalityDataService::adjustSatisfactionDelta($player->personality, -$satisfactionChange, 'not_played');
				$columns['w_zufriedenheit'] = min(100, max(1, $player->strengthSatisfaction + $satisfactionDelta));
			}
			...
			if (!$isTie) {
				if ($isTeamWinner) {
					$satisfactionDelta = PlayerPersonalityDataService::adjustSatisfactionDelta($player->personality, $satisfactionChange, 'win');
					$columns['w_zufriedenheit'] = min(100, max(1, $columns['w_zufriedenheit'] + $satisfactionDelta));
				} else {
					$satisfactionDelta = PlayerPersonalityDataService::adjustSatisfactionDelta($player->personality, -$satisfactionChange, 'loss');
					$columns['w_zufriedenheit'] = min(100, max(1, $columns['w_zufriedenheit'] + $satisfactionDelta));
				}
			}
		...
		if (!$player->team->isNationalTeam && $playerinfo['lending_matches'] > 0) {
			$this->handleBorrowedPlayer($columns, $playerinfo, $match, $player);
		}
	...
	private function updatePlayerWhoDidNotPlay(SimulationMatch $match, $isNationalTeam, $playerinfo) {
		...
		if (!$isNationalTeam || $this->_websoccer->getConfig('sim_playerupdate_through_nationalteam')) {
			$satisfactionDelta = PlayerPersonalityDataService::adjustSatisfactionDelta($playerinfo['personality'], -$satisfactionChange, 'not_played');
			$columns['w_zufriedenheit'] = min(100, max(1, $playerinfo['w_zufriedenheit'] + $satisfactionDelta));
		...
		if (!$isNationalTeam && $playerinfo['lending_matches'] > 0) {
			$this->handleBorrowedPlayer($columns, $playerinfo, $match, NULL);
		}
	...
	private function creditSponsorPayments(SimulationMatch $match, SimulationTeam $team, $isHomeTeam, $teamIsWinner) {
		
		$sponsor = SponsorsDataService::getSponsorinfoByTeamId($this->_websoccer, $this->_db, $team->id);
		if (!$sponsor) {
			return;
		}

		$amount = (int) $sponsor['amount_match'];
		
		if ($isHomeTeam) {
			$amount += (int) $sponsor['amount_home_bonus'];
			$amount += SponsorsDataService::calculateAttendanceBonusForMatch(
				$this->_websoccer,
				$this->_db,
				$match->id,
				$team->id,
		...
		if ($teamIsWinner) {
			$amount += (int) $sponsor['amount_win'];
		}

		if ($match->type == 'Pokalspiel') {
			$amount += (int) $sponsor['amount_cup'];
		}
		...
		// Dynamic sponsor contracts run until season completion and are not reduced per match.
		// Keep the legacy countdown only for old contracts without a contract snapshot.
		if (!isset($sponsor['contract_id']) || (int) $sponsor['contract_id'] <= 0) {
			$updatecolums['sponsor_spiele'] = max(0, (int) $sponsor['matchdays'] - 1);
	...
	private function handleBorrowedPlayer(&$columns, $playerinfo, SimulationMatch $match, $player = NULL) {
		...
		if ($columns['lending_matches'] == 0) {
			...
			$loanStatus = LoanDataService::handleEndOfLoan($this->_websoccer, $this->_db, $columns, $playerinfo);
			if ($loanStatus == LoanDataService::STATUS_BOUGHT) {
				return;
			}
