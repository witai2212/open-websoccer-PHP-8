...
class DbConnection {
	...
	public function executeQuery($queryStr) {
		...
		// Raw UPDATE/INSERT/DELETE queries bypass queryUpdate/queryInsert/queryDelete.
		// Clear the in-request SELECT cache as soon as data changes, otherwise pages can
		// display stale values after recalculating player strength/market value.
		if (preg_match('/^\s*(UPDATE|INSERT|DELETE|REPLACE|ALTER|CREATE|DROP|TRUNCATE)\b/i', $queryStr)) {
			$this->_queryCache = array();
		}
