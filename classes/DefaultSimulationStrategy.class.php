...
class DefaultSimulationStrategy implements ISimulationStrategy {
    ...
    public function nextAction(SimulationMatch $match) {
        ...
        if (class_exists('TacticalStyleDataService')) {
            $pAction['tackle'] = max(1, min(45, $pAction['tackle'] + TacticalStyleDataService::getTackleProbabilityEffect($opponentTeam)));
        }
        ...
        // team chemistry slightly improves/reduces stability in tense situations
        if (class_exists('TeamChemistryDataService')) {
            $chemistryStability = TeamChemistryDataService::getTeamStabilityEffect($player->team);
            if ($chemistryStability != 0 && $player->team->getGoals() <= $opponentTeam->getGoals()) {
                $resultInfluence += $chemistryStability;
            }
        }
        
        $styleShootInfluence = 0;
        if (class_exists('TacticalStyleDataService')) {
        ...
        $pAction['shoot'] = round(max($minShootProb, min($pShoot + $tacticInfluence + $resultInfluence + $styleShootInfluence, 50)) * $this->_websoccer->getConfig('sim_shootprobability') / 100);
    ...
    public function passBall(SimulationMatch $match) {
        ...
        $passingStrength = $player->strengthPassing;
        //old: $pFailed[FALSE] = round(($player->getTotalStrength($this->_websoccer, $match) + $player->strengthTech) / 2);
        $pFailed[FALSE] = round(($player->getTotalStrength($this->_websoccer, $match) + $passingStrength) / 2);
        ...
        // team chemistry has only a very small passing effect (-3..+3 by default)
        if (class_exists('TeamChemistryDataService')) {
    ...
    public function tackle(SimulationMatch $match) {
        ...
        $tacklingA = $player->strengthTackling;
        $opponentTackle = $opponent->strengthTackling;
        
        //$x = $player->getTotalStrength($this->_websoccer, $match);
        ...
        if ($result == TRUE) {
            ...
            //old: $pTackle['yellow'] = round(max(1, min(20, round((100 - $opponent->strengthTech) / 2))) * $this->_websoccer->getConfig('sim_cardsprobability') / 100);
            $pTackle['yellow'] = round(max(1, min(20, round((100 - $opponentTackle) / 2))) * $this->_websoccer->getConfig('sim_cardsprobability') / 100);
            ...
            if ($tackled == 'yellow' || $tackled == 'red') {
                ...
                $injuryProbability = ((100 - $player->strengthFreshness) / 3) * $this->_websoccer->getConfig('sim_injuredprobability') / 100;
                $injuryProbability = $injuryProbability * PlayerPersonalityDataService::getInjuryProbabilityMultiplier($player->personality);
                $pInjured[TRUE] = min(99, round($injuryProbability));
                ...
                if ($injured) {
                    ...
                    if (!isset($pInjuredMatches[$maxMatchesInjured])) {
                ...
                if ($player->position == PLAYER_POSITION_STRIKER) {
                    ...
                    //old: $shootStrength = $freeKickScorer->getTotalStrength($this->_websoccer, $match);
                    $shootStrength = $freeKickScorer->strengthFreekick;
    ...
    public function shoot(SimulationMatch $match) {
        ...
        //old: $shootStrength = round($player->getTotalStrength($this->_websoccer, $match) * $this->_shootStrengthPerPosition[$player->position] / 100);
        $playerShootStrength = $player->strengthShooting;
        $shootStrength = round($player->getTotalStrength($this->_websoccer, $match) * $playerShootStrength / 100);
        ...
        if ($player->position != PLAYER_POSITION_STRIKER ||
            ...
            if ($result == FALSE) {
                ...
                if (SimulationHelper::selectItemFromProbabilities($pCorner)) {
                    ...
                    // select players
                    if ($player->team->cornerPlayer) {
                        $passingPlayer = $player->team->cornerPlayer;
                    } else if ($player->team->freeKickPlayer) {
    ...
    private function _shootPenalty(SimulationMatch $match, SimulationPlayer $player) {
        ...
        $goaly = SimulationHelper::selectPlayer(
            SimulationHelper::getOpponentTeam($player, $match),
            PLAYER_POSITION_GOALY,
            null
            );
        
        // Penalty saving ability of the goalkeeper reduces the scorer's chance.
        // Important: the existing SimulationPlayer property is named "strengthPenaltyKilling".
        $goalyPenKilling = $goaly->strengthPenaltyKilling;
        $shootReduction = round(
            $goaly->getTotalStrength($this->_websoccer, $match) * $goalyPenKilling / 100
            );
