...
class DefaultSimulationStrategy implements ISimulationStrategy {
	...
	private function _shootPenalty(SimulationMatch $match, SimulationPlayer $player) {
	    ...
	    //echo"<b>Penalty: ". $player['strengthPenalty'] ."</b><br>";
		//echo"<b>GOALY: ". $player['strengthPenaltyKilling'] ."</b><br>";
		
		// get goaly influence from settings. 20 = 20%
		//old: $goalyInfluence = (int) $this->_websoccer->getConfig('sim_goaly_influence');
		$shootReduction = round($goaly->getTotalStrength($this->_websoccer, $match) * $goalyInfluence/100);
		$goalyPenKilling = $goaly->strengthPenalytKilling;
		$shootReduction = round($goaly->getTotalStrength($this->_websoccer, $match) * $goalyPenKilling/100);
