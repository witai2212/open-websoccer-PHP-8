...
class Simulator {
    ...
    public function simulateMatch(SimulationMatch $match, $minutes) {
    	...
    	// Determine regular match end minute only once.
    	// It must remain stable across multiple simulation runs.
    	if ($match->regularEndMinute < 91 || $match->regularEndMinute > 95) {
    	    $match->regularEndMinute = 90 + SimulationHelper::getMagicNumber(1, 5);
    	}
    	
    	$regularEndMinute = $match->regularEndMinute;
    	...
    	for ($simMinute = 1; $simMinute <= $minutes; $simMinute++) {
    		...
    		// old: $lastMinute = 90 + SimulationHelper::getMagicNumber(1, 5);
    		if ($match->penaltyShootingEnabled || $match->type == 'Pokalspiel') {
    			...
    			// regular match
    		} elseif ($match->minute >= $regularEndMinute) {
    			$this->completeMatch($match);
    			break;
    		}
