...
/**
 * Signs a temporary scouting proposal as a real player.
 */
class AcceptScoutingProposalController implements IActionController {
    private $_i18n;
    private $_websoccer;
    private $_db;
    ...
    public function executeAction($parameters) {
        ...
        $proposalId = isset($parameters['id']) ? (int) $parameters['id'] : 0;
        if ($proposalId < 1) {
            throw new Exception('Illegal ID');
        }

        try {
            ScoutingDataService::acceptProposal($this->_websoccer, $this->_db, $proposalId, $teamId);
        } catch (Exception $e) {
        ...
        $this->_websoccer->addFrontMessage(new FrontMessage(
            ...
            $this->_i18n->getMessage('accept_scouting_proposal_message'),
            ''
        ));
