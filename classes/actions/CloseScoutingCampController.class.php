...
/**
 * Closes a scouting camp of the manager's club.
 */
class CloseScoutingCampController implements IActionController {
    private $_i18n;
    private $_websoccer;
    private $_db;
    ...
    public function executeAction($parameters) {
        ...
        $campId = isset($parameters['id']) ? (int) $parameters['id'] : 0;
        if ($campId < 1) {
            throw new Exception('Illegal ID');
        }

        try {
            ScoutingDataService::closeCamp($this->_websoccer, $this->_db, $campId, $teamId);
        } catch (Exception $e) {
        ...
        $this->_websoccer->addFrontMessage(new FrontMessage(
            ...
            $this->_i18n->getMessage('close_scouting_camp_message'),
            ''
        ));
