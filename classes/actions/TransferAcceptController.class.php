	...
	public function executeAction($parameters) {
		...
		if($hasUser>0 && $offer['user_id']>0) {
    		...
    		NotificationsDataService::createNotification($this->_websoccer, $this->_db, $offer["user_id"], 
    			  "player_bought_message",
    			 array("playername" => $playerName, "receivername" => $this->_websoccer->getUser()->username), 
    				  "transferoffer", "myteam");
		}
		
		//debit amount from buyer (take money)
		BankAccountDataService::debitAmount($this->_websoccer, $this->_db, $offer['verein_id'], $offer['abloese'], "player_bought_message", $oldTeam['team_name']);
		
		//credit amount to selle (give money)
		BankAccountDataService::creditAmount($this->_websoccer, $this->_db, $oldTeam['team_id'], $offer['abloese'], "player_sold_message", $newTeam['team_name']);
