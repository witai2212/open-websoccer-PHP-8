...
class StadiumEnvironmentPlugin {
	...
	/**
	 * Returns the sum of active completed building effects.
	 * If a completed building requires another completed building, the successor
	 * replaces the ongoing effects of its direct predecessor.
	 */
	private static function getBonusSumFromBuildings(WebSoccer $websoccer, DbConnection $db, $attributeName, $teamId) {
		...
		$buildingTable = $dbPrefix . '_stadiumbuilding';
		$fromTable = $buildingsOfTeamTable . ' AS BT INNER JOIN ' . $buildingTable . ' AS B ON B.id = BT.building_id';
		$where = 'BT.team_id = %d AND BT.construction_deadline < %d'
			. ' AND NOT EXISTS ('
			. 'SELECT 1 FROM ' . $buildingsOfTeamTable . ' AS BT2'
			. ' INNER JOIN ' . $buildingTable . ' AS B2 ON B2.id = BT2.building_id'
			. ' WHERE BT2.team_id = BT.team_id'
			. ' AND BT2.construction_deadline < %d'
			. ' AND B2.required_building_id = BT.building_id'
			. ')';

		$result = $db->querySelect(
			'SUM(B.' . $attributeName . ') AS attrSum',
			$fromTable,
			$where,
			array($teamId, $now, $now)
		);
		$resArray = $result->fetch_array();
		$result->free();

		if ($resArray && $resArray['attrSum'] !== NULL) {
