...
/******************************************************
  ...
  CM23 Task 1026 | 08.09.2026 | Revision 1
...
class SeasonRolloverWorkflowService {
    ...
    private static $steps = array(
        ...
        'league_schedules' => '6. Liga-Spieltage erstellen'
    );
    ...
    public static function executeNextStep(WebSoccer $websoccer, DbConnection $db, I18n $i18n, $runId, $confirmed) {
        ...
        try {
            if ($stepId === 'league_schedules') {
                ...
                $result = self::executeRegularStep($websoccer, $db, $i18n, $stepId, $state['options']);
                ...
                $state['current_step_index'] = $stepIndex + 1;
                $state['status'] = 'ready';
                self::log($runId, $stepId, 'done', 'Schritt erfolgreich abgeschlossen.', self::summarizeResult($result));
            }
    ...
    private static function executeRegularStep(WebSoccer $websoccer, DbConnection $db, I18n $i18n, $stepId, array $options) {
        switch ($stepId) {
            case 'end_seasons':
            case 'uefa_temp':
            case 'new_seasons':
                return SeasonRolloverDataService::executeStep($websoccer, $db, $i18n, $stepId, $options);
    ...
    private static function executeLeagueScheduleBatch(WebSoccer $websoccer, DbConnection $db, array $state) {
        ...
        $progress = isset($state['steps'][$stepId]['progress']) && is_array($state['steps'][$stepId]['progress'])
            ? $state['steps'][$stepId]['progress']
            : null;
        ...
        if (empty($remaining)) {
            return array(
                ...
                'done' => true,
                'progress' => $progress,
                'result' => array('batch_processed' => 0, 'batch_created_matches' => 0)
            );
        }
        ...
        foreach ($batchIds as $seasonId) {
            ...
            $result = self::generateSingleLeagueSchedule(
                $websoccer,
                $db,
                $season,
                (int) $progress['first_league_friday'],
                (int) $state['options']['league_rounds'],
                (int) $progress['maximum_matchdays']
            );
        ...
        return array(
            ...
            'progress' => $progress,
            'result' => array(
                'batch_processed' => count($batchProcessed),
                'batch_processed_ids' => $batchProcessed,
                'batch_skipped' => $batchSkipped,
                'batch_created_matches' => $batchCreatedMatches
            )
        );
    }
    ...
    private static function initializeLeagueScheduleProgress(WebSoccer $websoccer, DbConnection $db, array $options) {
        ...
        foreach ($seasons as $season) {
            ...
            $matchdays = SeasonRolloverScheduleService::getLeagueMatchdayCountForTeamCount(
                count($teamIds),
                (int) $options['league_rounds']
            );
            if ($matchdays > $maximumMatchdays) {
                $maximumMatchdays = $matchdays;
            }
    ...
    private static function generateSingleLeagueSchedule(WebSoccer $websoccer, DbConnection $db, array $season, $firstLeagueFriday, $rounds, $maximumMatchdays) {
        ...
        $existingResult = $db->querySelect(
            'COUNT(*) AS hits',
            $prefix . '_spiel',
            "saison_id = %d AND spieltyp = 'Ligaspiel'",
            $seasonId,
            1
        );
        ...
        if ($existingRow && (int) $existingRow['hits'] > 0) {
            return array(
                'created_matches' => 0,
                'skipped' => 1,
                'reason' => 'Spielplan war bereits vorhanden; Liga wurde nicht erneut verarbeitet.'
            );
        }
        ...
        try {
            foreach ($fullSchedule as $matchdayIndex => $matches) {
                ...
                if ($matchdayCount <= 1 || $maximumMatchdays <= 1) {
                    ...
                    $weekIndex = (int) round(
                        $matchdayIndex * ($maximumMatchdays - 1) / ($matchdayCount - 1)
                    );
                }

                $matchweekFriday = SeasonRolloverScheduleService::addDays(
                    $firstLeagueFriday,
                    $weekIndex * 7,
                    SeasonRolloverScheduleService::LEAGUE_KICKOFF_HOUR,
                    SeasonRolloverScheduleService::LEAGUE_KICKOFF_MINUTE
    ...
    private static function findLeagueMatchTimestamp(WebSoccer $websoccer, DbConnection $db, array $teamIds, $matchweekFriday, $preferredOffset, $isFinalMatchday) {
        ...
        if ($isFinalMatchday) {
            $candidate = SeasonRolloverScheduleService::addDays(
                $matchweekFriday,
                4,
                SeasonRolloverScheduleService::LEAGUE_KICKOFF_HOUR,
                SeasonRolloverScheduleService::LEAGUE_KICKOFF_MINUTE
            ...
            if (
                SeasonRolloverScheduleService::isConfiguredCupRoundDay($websoccer, $db, $candidate)
                || SeasonRolloverScheduleService::teamsHaveMatchOnDay($websoccer, $db, $teamIds, $candidate)
            ) {
    ...
    public static function createBackup(WebSoccer $websoccer, DbConnection $db, $backupId = null) {
            ...
            foreach ($tables as $tableInfo) {
                ...
                while ($row = $dataResult->fetch_assoc()) {
                    ...
                    if (count($rowBatch) >= 100) {
                        self::writeBackupSql(
                            $handle,
                            'INSERT INTO ' . $quotedTable . ' (' . implode(',', $columnNames) . ') VALUES ' . implode(',', $rowBatch)
                        );
                        $rowBatch = array();
                    }
                }
        ...
        $meta = array(
            ...
            'table_count' => count(array_filter($tables, function($tableInfo) {
                return $tableInfo['type'] === 'BASE TABLE';
            }))
        );
    ...
    public static function restoreBackup(WebSoccer $websoccer, DbConnection $db, $backupId) {
        ...
        self::log($backupId, 'restore', 'done', 'Datenbank-Backup wurde wiederhergestellt.', array(
            'backup_id' => $backupId,
            'safety_backup_id' => $safetyBackup['id']
        ));

        return array(
            'restored_backup' => $meta,
        ...
        foreach ($value as $key => $item) {
            ...
            if ($key === 'errors' && is_array($item) && !empty($item)) {
                foreach ($item as $error) {
                    if ((string) $error !== '') {
                        $errors[] = $currentPath . ': ' . (string) $error;
                    }
                }
                continue;
            }
            ...
            if (is_array($item)) {
                $errors = array_merge($errors, self::collectResultErrors($item, $currentPath));
            }
        }
        return $errors;
    }
    ...
    private static function summarizeResult($value) {
        ...
        foreach ($value as $key => $item) {
            if (is_array($item)) {
                $summary[$key] = count($item);
            } elseif (is_bool($item)) {
                $summary[$key] = $item ? 1 : 0;
            } else {
                $summary[$key] = $item;
            }
        }
    ...
    private static function saveRun(array $state) {
        ...
        if (!$runId) {
            throw new Exception('Ungültige Saisonwechsel-Lauf-ID.');
        }
        $dirs = self::ensureStorage();
    ...
    private static function writeJsonFile($file, array $data) {
        ...
        if ($json === false) {
            throw new Exception('Saisonwechsel-Status konnte nicht serialisiert werden.');
        }
        $tmp = $file . '.tmp';
        if (@file_put_contents($tmp, $json, LOCK_EX) === false) {
            throw new Exception('Saisonwechsel-Status konnte nicht gespeichert werden.');
        }
        if (!@rename($tmp, $file)) {
            @unlink($tmp);
            throw new Exception('Saisonwechsel-Status konnte nicht atomar gespeichert werden.');
        }
    }
    ...
    private static function getPrefixedTables(DbConnection $db, $prefix) {
        ...
        while ($row = $result->fetch_array(MYSQLI_NUM)) {
            ...
            if (strpos($name, $prefix . '_') === 0) {
                $tables[] = array('name' => $name, 'type' => $type);
            }
        }
        $result->free();
        return $tables;
