 ...
 */
class UEFAEuroLeagueDataService {
	 ...
	 */
	public static function getELData(WebSoccer $websoccer, DbConnection $db) {
		
		$sqlStr = "SELECT C.* FROM ". $websoccer->getConfig("db_prefix") ."_cup AS C WHERE C.name = 'UEFA Euro League'";
	 ...
	 * @return return array of Champions league data.
	 */
	public static function getELGroupDataByGroup(WebSoccer $websoccer, DbConnection $db, $cup_round_id, $group_name) {
			    
		$group = array();
		
		$sqlStr = "SELECT CR.*, CG.*, V.id AS club_id, V.name AS club_name, L.land AS land
					...
					ORDER BY CG.tab_points, CG.tab_wins, CG.tab_goals, CG.tab_draws, CG.tab_losses DESC, CG.tab_goalsreceived DESC";
	    $result = $db->executeQuery($sqlStr);
	...
	public static function getElGroupId(WebSoccer $websoccer, DbConnection $db, $elId) {
		
		$sqlStr = "SELECT CR.id FROM ". $websoccer->getConfig("db_prefix") ."_cup_round AS CR
					WHERE CR.name='Gruppen' AND CR.cup_id='$elId'";
	    $result = $db->executeQuery($sqlStr);
	...
	public static function getElGroupIdByName(WebSoccer $websoccer, DbConnection $db, $grId, $grName) {
	 ...
	 * @return return array of Champions league data.
	 */
	public static function getELGroups(WebSoccer $websoccer, DbConnection $db, $cup_round_id) {
	 ...
	 * 
	 */
	public static function getELMatchesByRound(WebSoccer $websoccer, DbConnection $db, $cup_name, $cup_round, $cup_group) {
	    ...
	    $sqlStr = "SELECT M.*, HT.name AS home_team_name, 
                        HT.liga_id AS home_team_ligaid, AT.name AS away_team_name, AT.liga_id AS away_team_ligaid,
                        HL.land, AL.land
                    ...
                    WHERE M.spieltyp='Pokalspiel'
                        AND M.pokalname='$cup_name' AND M.pokalrunde='$cup_round'
                    ORDER BY datum ASC";
	    $result = $db->executeQuery($sqlStr);
