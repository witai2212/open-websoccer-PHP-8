...
class UefaDataService {
	...
	*/
	 ...
	 * @return array Array with teams in uefa temp by cup
	 */
	public static function getUefaTeamsByGroup(WebSoccer $websoccer, DbConnection $db, $group_name) {
		...
		// get uefa place from land
	    $queryString = "SELECT team_id FROM ". $websoccer->getConfig('db_prefix') ."_cup_round_group WHERE name='$group_name' ORDER BY RAND()";
	    $result = $db->executeQuery($queryString);
	    while ($team = $result->fetch_array()) {
