...
function getNation() {
    ...
    $nameStr = "SELECT name FROM ". $conf["db_prefix"] ."_land ORDER BY RAND() LIMIT 1";
    //echo $nameStr ."<br>";
    $nameQuery = $db->executeQuery($nameStr)->fetch_array();
    $land = $nameQuery['name'];
...
function generateScouts() {
    ...
    for($i=1;$i<=5;$i++) {
        ...
        for($j=1;$j<=8;$j++) {
            ...
            $insStr = "INSERT INTO ". $conf["db_prefix"] ."_scout (name, nation, expertise, fee, speciality) VALUES ('$name', '$nation', '$kompetenz', '$kosten', '$position')";
			//echo $insStr ."<br>";
