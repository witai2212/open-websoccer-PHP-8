...
class ComputerTransfersDataService {
    ...
    private static function placeBid(WebSoccer $websoccer, DbConnection $db, $teamId, $playerId, $bidAmount, $now, $salaryAmount, $goalAmount) {
        ...
        $isFreeTransfer = ((int) $playerData['team_id'] < 1);
        $transferFee = $isFreeTransfer ? 0 : $bidAmount;
        $handgeld = 0;
        if ($isFreeTransfer) {
            $rand = rand(10, 50) / 100;
        ...
        $insStr = "INSERT INTO ". $websoccer->getConfig('db_prefix') . "_transfer_angebot
                    (spieler_id, verein_id, user_id, abloese, handgeld, vertrag_spiele, datum, vertrag_gehalt, vertrag_torpraemie, ishighest)
                    VALUES ('$playerId', '$teamId', NULL, '$transferFee', '$handgeld', '60', '$now', '$salaryAmount', '$goalAmount', '0')";
        $db->executeQuery($insStr);
        TransfermarketDataService::updateHighestOffer($websoccer, $db, $playerId);
    ...
    public static function deleteTooManyOffers(WebSoccer $websoccer, DbConnection $db, $maxOffersPerPlayer = 6) {
        ...
        foreach ($playerIds as $playerId) {
            ...
            $cpuQuery = "SELECT A.id FROM " . $table . " AS A
                         INNER JOIN " . $playerTable . " AS P ON P.id = A.spieler_id
                         WHERE A.spieler_id='" . $playerId . "'
                           AND (A.user_id IS NULL OR A.user_id <= 0)
                         ORDER BY CASE WHEN COALESCE(P.verein_id, 0) > 0
                             THEN A.abloese
                             ELSE (A.handgeld + (A.vertrag_spiele * A.vertrag_gehalt)
                                 + CASE WHEN P.position IN ('Mittelfeld', 'Sturm') THEN (10 * A.vertrag_torpraemie) ELSE 0 END)
                             END DESC,
                             A.datum ASC, A.id ASC";
