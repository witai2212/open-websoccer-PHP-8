...
class TransfermarketDataService {
	...
	public static function getHighestBidForPlayer(WebSoccer $websoccer, DbConnection $db, $playerId) {
		...
		$columns['COALESCE(P.verein_id, 0)'] = 'player_team_id';
		...
		// Players under contract are ranked only by the transfer fee. For a genuine
		// free transfer, the complete contract package decides the auction.
		$rankingExpression = "CASE WHEN COALESCE(P.verein_id, 0) > 0
			THEN B.abloese
			ELSE (B.handgeld + (B.vertrag_spiele * B.vertrag_gehalt)
				+ CASE WHEN P.position IN ('Mittelfeld', 'Sturm') THEN (10 * B.vertrag_torpraemie) ELSE 0 END)
			END";
		$whereCondition = 'B.spieler_id = %d ORDER BY ' . $rankingExpression . ' DESC, B.datum ASC, B.id ASC';
	...
	public static function transferPlayer(WebSoccer $websoccer, DbConnection $db, $player, $bid) {
		...
		// Enforce the financial separation at execution time as well. This also
		// protects ongoing auctions containing older or manually inserted offers.
		$isFreeTransfer = ((int) $player['team_id'] < 1);
		if ($isFreeTransfer) {
			$bid['amount'] = 0;
		} else {
			$bid['hand_money'] = 0;
	...
	public static function updateHighestOffer(WebSoccer $websoccer, DbConnection $db, $spieler_id) {
		...
		// Determine the highest offer according to the same rules used at auction
		// completion: transfer fee only for contracted players, contract package for
		// free agents.
		$playerTable = $websoccer->getConfig('db_prefix') . '_spieler';
		$fromTable = $table . ' AS B INNER JOIN ' . $playerTable . ' AS P ON P.id = B.spieler_id';
		$rankingExpression = "CASE WHEN COALESCE(P.verein_id, 0) > 0
			THEN B.abloese
			ELSE (B.handgeld + (B.vertrag_spiele * B.vertrag_gehalt)
				+ CASE WHEN P.position IN ('Mittelfeld', 'Sturm') THEN (10 * B.vertrag_torpraemie) ELSE 0 END)
			END";
		$result = $db->querySelect('B.id', $fromTable, 'B.spieler_id = %d ORDER BY ' . $rankingExpression . ' DESC, B.datum ASC, B.id ASC LIMIT 1', $spieler_id);
